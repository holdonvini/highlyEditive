...
import { SingleSimplePricing } from "./single-simple-pricing";
import { SingleComplexPricing } from "./single-complex-pricing";
import { ArrowRight } from "@/core/icons";

type SingleEditPricingType = keyof typeof PRICING.single;

const tabs = [
  { id: "simple", label: "Simple" },
  { id: "complex", label: "Complex" },
] satisfies { id: SingleEditPricingType; label: string }[];

interface SingleEditPricingProps {
  selectedPricingOption: string | null;
  onSelectPricingOption: (optionId: string) => void;
}

export function SingleEditPricing({
  selectedPricingOption,
  onSelectPricingOption,
}: SingleEditPricingProps) {
  const [activeTab, setActiveTab] = useState<SingleEditPricingType>("simple");
          ...
          >
            {tabs.map((tab) => (
              <Tabs.Tab
                ...
                data-slot="tabs-tab"
                className={cn(
                  "flex h-9 shrink-0 grow cursor-pointer items-center justify-center gap-1.5 whitespace-nowrap rounded-md border border-transparent px-[calc(--spacing(2.5)-1px)] font-medium text-base outline-none transition-[color,background-color,box-shadow] hover:text-foreground/80 data-active:bg-brand-light/10 data-active:text-brand-light",
                )}
              >
                {tab.label}
              </Tabs.Tab>
        ...
        {selectedPricingOptionData ? (
          <div className="flex flex-col gap-5 max-w-xs w-full p-4">
            ...
            <div className="w-full flex justify-center *:w-full">
              <button className="cursor-pointer inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-xl text-sm tracking-wide font-medium [transition-property:background-color,scale] duration-100 ease-[cubic-bezier(.25,.46,.45,.94)] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-foreground/50 shadow-md border-[0.5px] border-foreground/25 text-shadow-sm bg-gradient-to-b from-brand to-brand-light text-white hover:from-brand-light hover:to-brand px-4 py-2 active:scale-98 group">
